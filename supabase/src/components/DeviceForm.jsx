...
export default function DeviceForm({ mode, initial, defaultAddedby, onSubmit, onCancel, busy }) {
  ...
  const hasModels = typeModels.length > 0;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="الموديل *">
          {hasModels ? (
            <select className={sel} value={model}
                    onChange={(e) => { setModel(e.target.value); setF((p) => ({ ...p, color: '' })); setColorCustom(false); }}>
              <option value="">-- اختر --</option>
              {typeModels.map((m) => <option key={m} value={m}>{m}</option>)}
              {model && !typeModels.includes(model) && <option value={model}>{model}</option>}
            </select>
          ) : (
            <input
              className={sel}
