...
const MODULES = [
  ...
  { key:'sales', perm:'can_erp_sales', icon:'🧾', label:'المبيعات', items:[['sales-all','كل المبيعات'],['sales-add','إضافة بيع'],['pos','نقطة البيع'],['sales-returns','مرتجع المبيعات']] },
  ...
  { key:'products', perm:'can_erp_products', icon:'🏷️', label:'المنتجات', items:[['products','المنتجات'],['sale-price-groups','مجموعات أسعار البيع'],['product-movements','حركة صنف'],['units','الوحدات'],['categories','الأقسام'],['brands','العلامات التجارية'],['serials','السريلات'],['warranties','الضمانات']] },
...
export default function ERP(){
  ...
  useEffect(() => { if (!allowed.has(active)) setActive(visibleModules[0]?.items[0]?.[0] || 'overview'); }, [allowed, active, visibleModules]);
  ...
  return <div dir="rtl" className="py-5">
    ...
    <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_260px]">
      ...
      <aside className="min-w-0 overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        ...
        <div className="p-2">
          {visibleModules.map(m=> <div key={m.key} className="mb-1">
            ...
            {open===m.key && <div className="mr-5 mt-1 border-r border-border pr-2">{m.items.map(([k,l])=><button key={k} onClick={()=>setActive(k)} className={`mb-1 block w-full rounded-lg px-3 py-2 text-right text-xs font-bold transition ${active===k?'bg-accent text-on-accent':'text-muted hover:bg-surface hover:text-text'}`}>{l}</button>)}</div>}
          </div>)}
        </div>
      </aside>
    </div>
