...
export default function ProductLookupAdmin({table,title,symbol=false}){
  ...
  const hardRemoveCategory=async r=>{
    if(!category)return;
    if(!can('can_erp_reset')){setIsError(true);setMsg('الحذف الخشن للأقسام يحتاج صلاحية إعادة ضبط ERP.');return;}
    if(!confirm(`حذف خشن ونهائي للقسم "${r.name}"؟\nسيتم رفض العملية بالكامل إذا كان داخل القسم أي صنف له بيع أو شراء أو مرتجع.\nأما الأصناف بلا تاريخ بيع/شراء فسيتم حذفها مع بياناتها التشغيلية التابعة.`))return;
    const phrase=prompt(`للتأكيد اكتب: DELETE CATEGORY ${r.id}`);if(phrase!==`DELETE CATEGORY ${r.id}`){setIsError(true);setMsg('تم إلغاء الحذف: عبارة التأكيد غير صحيحة.');return;}
    setDeleting(r.id);setMsg('');setIsError(false);
    try{const{data,error}=await supabase.rpc('erp_category_hard_delete',{p_category_id:r.id,p_confirmation:phrase});if(error)throw error;if(editId===r.id)cancelEdit();await load();setMsg(`تم حذف القسم "${data?.category_name||r.name}" نهائيًا وحذف ${data?.deleted_products||0} صنف غير محمي بداخله.`)}
    catch(err){setIsError(true);setMsg(`تعذر الحذف الخشن: ${err.message}`)}finally{setDeleting(null)}
  };
  ...
  return <div className="max-w-3xl">
    ...
    <div className="overflow-hidden rounded-2xl border border-border">
      {rows.map(r=><div key={r.id} className="flex flex-wrap items-center justify-between gap-3 border-b border-border p-3 last:border-0">
        ...
        <div className="flex gap-2"><button type="button" onClick={()=>beginEdit(r)} disabled={!canEdit} className="rounded-lg border border-accent-line px-3 py-2 text-xs font-black text-accent disabled:opacity-40">تعديل</button><button type="button" onClick={()=>remove(r)} disabled={!canDelete||deleting===r.id} className="rounded-lg border border-red-500/30 px-3 py-2 text-xs font-black text-red-400 disabled:opacity-40">{deleting===r.id?'جارٍ الحذف…':'حذف'}</button>{category&&<button type="button" onClick={()=>hardRemoveCategory(r)} disabled={!can('can_erp_reset')||deleting===r.id} className="rounded-lg bg-red-700 px-3 py-2 text-xs font-black text-white disabled:opacity-40">حذف خشن</button>}</div>
      </div>)}
