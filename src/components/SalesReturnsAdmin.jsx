...
export default function SalesReturnsAdmin() {
  ...
  const loadRecent = () =>
    supabase.from('sales_returns').select('*').order('created_at', { ascending: false }).limit(100)
  ...
  const loadInvoices = async () => {
    ...
    const map = {};
    if (ids.length) {
      const { data: rets } = await supabase.from('sales_returns')
        .select('sales_invoice_id,status,sales_return_items(product_id,quantity)')
        .eq('status', 'posted').in('sales_invoice_id', ids);
      (rets || []).forEach(r => (r.sales_return_items || []).forEach(it => {
        map[r.sales_invoice_id] = map[r.sales_invoice_id] || {};
        map[r.sales_invoice_id][it.product_id] = (map[r.sales_invoice_id][it.product_id] || 0) + Number(it.quantity || 0);
      }));
    }
    setReturnedMap(map);
  };
  ...
  const post = async () => {
    ...
    try {
      ...
      if (settlementType === 'cash' && !treasury) throw Error('حدد الخزينة التي سيتم رد المبلغ منها.');
  ...
  return (
    <div className="rounded-2xl border border-border p-4">
      ...
      <select className={`${I} mt-3`} value={invoice} onChange={e => { setInvoice(e.target.value); setQty({}); setSerials({}); }}>
        ...
        {invoices.map(x => <option key={x.id} value={x.id}>{x.invoice_number} · {x.customer_name}</option>)}
      </select>
      {!branchesLoading && invoices.length === 0 && (
        <p className="mt-2 text-xs font-bold text-danger">لا توجد فواتير مبيعات قابلة للإرجاع ضمن فروعك المتاحة. (الفواتير المرتجعة بالكامل لا تظهر، ولو المفروض تشوف فواتير فعّل «عرض جميع الفروع» أو اربط المستخدم بالفرع.)</p>
      )}
      {invItems.map(x => (
        <div className="mt-2 grid gap-2 md:grid-cols-[1fr_120px_1fr]" key={x.id}>
          ...
          <textarea className={I} disabled={!x.products?.serial_tracked} value={serials[x.id] || ''} onChange={e => setSerials({ ...serials, [x.id]: e.target.value })} placeholder={x.products?.serial_tracked ? 'السريلات المرتجعة' : 'غير متتبع'} />
        </div>
      ))}
      ...
      <button disabled={busy || !can('can_erp_sale_return')} onClick={post} className="mt-3 rounded-xl bg-accent px-5 py-2.5 font-black text-on-accent disabled:opacity-50">{busy ? 'جارٍ الترحيل...' : 'ترحيل المرتجع'}</button>
        ...
        <div className="mt-6 rounded-2xl border border-border p-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[600px] text-sm">
              <thead><tr className="text-right text-xs text-muted"><th className="p-2">المستند</th><th>التاريخ</th><th>الإجمالي</th><th>الحالة</th></tr></thead>
              <tbody>
                {returnRows.map(r => (
                  <tr key={r.id} className="border-t border-border">
                    <td className="p-2"><button type="button" onClick={() => setViewDoc(r.id)} className="font-black text-accent underline underline-offset-4">{r.return_number}</button></td>
      ...
      {viewDoc && <ERPDocumentViewer type="sales_return" id={viewDoc} onClose={() => setViewDoc(null)} />}
    </div>
  );
}
