...
// خزائن المستخدم — V12
// الوصول يُحفظ في جدول مستقل، والخزينة الافتراضية واحدة فقط لكل مستخدم.
export async function fetchActiveTreasuries() {
  const { data, error } = await supabase
    .from('treasuries')
    .select('id,name,code,branch,is_active')
    .eq('is_active', true)
    .order('branch')
    .order('name');
  if (error) throw error;
...
export async function fetchUserTreasuryAccess(userId) {
  const { data, error } = await supabase
    .from('user_treasury_access')
    .select('treasury_id,is_default')
    .eq('user_id', userId);
  if (error) throw error;
  return data || [];
}

export async function setUserTreasuryAccess(userId, treasuryIds, defaultTreasuryId) {
  const clean = [...new Set((treasuryIds || []).map(Number).filter(Number.isFinite))];
  const def = defaultTreasuryId == null || defaultTreasuryId === '' ? null : Number(defaultTreasuryId);
  if (def != null && !clean.includes(def)) throw new Error('الخزينة الافتراضية يجب أن تكون ضمن الخزائن المسموح بها');

  const { error: delErr } = await supabase.from('user_treasury_access').delete().eq('user_id', userId);
  if (delErr) throw delErr;
  if (!clean.length) return;

  const { error } = await supabase.from('user_treasury_access').insert(
    clean.map((treasury_id) => ({ user_id: userId, treasury_id, is_default: treasury_id === def }))
  );
  if (error) throw error;
}
