...
// ── الموديلات المخصّصة (custom_phone_models) — V13.7.0 ──────────
// كتالوج مستقل عن ملف الـCSV المدمج. البحث بيدمج الاتنين، لكن
// الإضافة/التعديل بتتخزّن هنا بس — فتحديث الـCSV مستقبلًا ما بيمسّش
// أي موديل أضافه المستخدم.
//
// scope: 'global' → يظهر لكل الفروع · 'local' → لفرع المستخدم بس.
// المطابقة/منع التكرار بتتحسب على السيرفر كمان (فهرس فريد مطبّع)،
// وهنا بنعمل فحص مبدئي لتجربة استخدام أحسن.
...
/**
 * قراءة الموديلات المخصّصة المرئية للمستخدم.
 * الـRLS بتقيّد الرؤية (global + فرع المستخدم)، فبنقرا الكل النشط
 * والباقي على السيرفر.
 */
export async function fetchCustomModels() {
  const { data, error } = await supabase
    .from('custom_phone_models')
    .select('id,brand,model,scope,branch_id,is_active')
    .eq('is_active', true)
    .order('brand')
    .order('model');
  if (error) return [];
...
/**
 * إضافة موديل مخصّص جديد.
 * @param {{brand:string, model:string, scope:'global'|'local', branchId?:string|null}} p
 * @returns {Promise<{ok:true,row:object} | {ok:false,duplicate?:boolean,error?:string}>}
 */
export async function addCustomModel({ brand, model, scope = 'global', branchId = null }) {
  ...
  // فحص تكرار مبدئي ضد المخصّص الموجود (السيرفر هو الحكم النهائي)
  const { data: dup } = await supabase
    ...
    .eq('model_key', key)
    .limit(1);
  if (dup && dup.length) return { ok: false, duplicate: true };
  ...
  if (error) {
    // 23505 = انتهاك قيد الفريد (تكرار على مستوى السيرفر)
