...
export default function ShopProductDetails(){
 ...
 const imgs=p.images||[];
 return <div className="min-h-screen bg-bg text-text" dir="rtl"><header className="sticky top-0 z-30 border-b border-border bg-card/95 p-4 backdrop-blur"><div className="mx-auto flex max-w-6xl items-center justify-between"><button onClick={()=>nav('/')} className="text-xl font-black text-accent">APP TECH</button><button onClick={()=>nav(-1)} className="text-sm font-black">← رجوع</button></div></header><main className="mx-auto grid max-w-6xl gap-6 px-4 py-6 md:grid-cols-2"><div><div className="aspect-square overflow-hidden rounded-3xl border border-border bg-surface">{imgs[image]?<img src={publicMediaUrl('product-images',imgs[image])} alt={p.name} className="size-full object-contain"/>:<div className="grid size-full place-items-center text-7xl opacity-30">📦</div>}</div>{imgs.length>1&&<div className="mt-3 flex gap-2 overflow-x-auto">{imgs.map((x,i)=><button key={x} onClick={()=>setImage(i)} className={`size-16 shrink-0 overflow-hidden rounded-xl border ${i===image?'border-accent':'border-border'}`}><img src={publicMediaUrl('product-images',x)} className="size-full object-cover"/></button>)}</div>}</div><div className="rounded-3xl border border-border bg-card p-5 md:p-7"><p className="text-xs font-bold text-muted">{p.product_brands?.name||'APP TECH'}</p><h1 className="mt-2 text-2xl font-black md:text-3xl">{p.name}</h1>{p.sku&&<p className="mt-2 text-xs text-muted">كود المنتج: {p.sku}</p>}<div className="mt-5 grid gap-3"><div className="rounded-xl border border-border bg-surface p-3"><span className="text-xs text-muted">القسم</span><b className="mt-1 block">{p.product_categories?.name||'—'}</b></div>{isAdmin()&&stock!==null&&<div className="rounded-xl border border-accent-line bg-accent-soft p-3"><span className="text-xs text-muted">الرصيد الحالي — للإدارة فقط</span><b className="mt-1 block text-lg text-accent">{Number(stock.toFixed(3)).toLocaleString('ar-EG')}</b></div>}{p.notes&&<div className="rounded-xl border border-border bg-surface p-3"><span className="text-xs text-muted">التفاصيل</span><p className="mt-1 whitespace-pre-wrap text-sm leading-7">{p.notes}</p></div>}</div><ShareBar url={`${window.location.origin}/product/${p.id}`} text={p.name} /><button onClick={()=>nav(`/products${p.category_id?`?category=${p.category_id}`:''}`)} className="mt-5 w-full rounded-xl border border-accent-line bg-accent-soft py-3 text-sm font-black text-accent">عرض منتجات القسم</button></div></main></div>
}
