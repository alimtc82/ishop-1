...
export const stockQty=(movements,productId,branch='')=>(movements||[]).filter(m=>String(m.product_id)===String(productId)&&(!branch||String(m.branch||'').toLocaleLowerCase()===String(branch).toLocaleLowerCase())).reduce((s,m)=>{
